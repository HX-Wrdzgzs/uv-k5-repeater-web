...
export const onRequestPost: PagesHandler = async ({ request, env }) => {
  ...
  if (env.RESEND_API_KEY) {
    const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${env.RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: '全国中继 <noreply@mizuki.top>', to: [email], subject: '你的全国中继登录链接', html: `<p>点击下面的链接登录全国中继数据协作站：</p><p><a href="${verifyUrl}">登录全国中继</a></p><p>链接 15 分钟内有效且只能使用一次。</p>` }) })
