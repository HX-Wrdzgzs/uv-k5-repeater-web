...
import { assertSameOrigin, audit, error, json, requireUser, now } from '../../_shared/http'
...
export const onRequestPost: PagesHandler = async ({ request, env }) => {
  ...
  try {
    ...
    const current = now()
    await env.DB.prepare('INSERT INTO submissions (id, user_id, kind, payload_json, status, risk, validation_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(id, user.id, duplicate ? 'update' : 'create', JSON.stringify({ ...input, stationKey: key }), 'pending', risk, '[]', current, current).run()
    await audit(env, user.id, 'submission.created', 'submission', id, null, { ...input, risk, status: 'pending' }, request)
